...
type LatestAssessmentsResponse = {
  items: AssessmentRunLatestItem[];
};

type AssessmentTypeResult = {
...
type ProposalReference = {
  ...
  target_planning_level: string;
  proposal_title: string;
};

type DialogContextEntry = {
  dialog_context_id?: number;
  assessment_run_id?: number;
  proposal_id?: number;
  entry_kind: string;
  entry_scope: string;
  ...
  entry_text: string;
  linked_evidence_json?: string | null;
...
type AssessmentRunDetail = {
  ...
  assessment_type_results: AssessmentTypeResult[];
  findings: AssessmentFinding[];
  proposals: ProposalReference[];
  dialog_context: DialogContextEntry[];
};
...
type ProposalDecision = {
  proposal_decision_id: number;
  decision_status: string;
  decision_note?: string | null;
  decided_by: string;
  decided_at: string;
  superseding_proposal_id?: number | null;
...
type ProposalDetail = {
  ...
  dialog_context: DialogContextEntry[];
  decision_history: ProposalDecision[];
};
...
function toDialogScopeLabel(value: string) {
  if (value === "assessment_summary") {
    return "Resumen de evaluacion";
  }
  if (value === "reassessment_request") {
    return "Solicitud de reevaluacion";
  }
  return toHumanLabel(value);
}

function toClarificationKindLabel(value: string | null | undefined) {
  if (value === "schedule_shift") {
    return "Cambio de calendario";
  }
  if (value === "session_swap") {
    return "Intercambio de sesiones";
  }
  if (value === "missing_context") {
    return "Contexto faltante";
  }
  if (value === "device_issue") {
    return "Incidencia de dispositivo";
  }
  if (value === "execution_intent") {
    return "Intencion de ejecucion";
  }
  return value ? toHumanLabel(value) : "Sin clasificar";
}

function parseJsonStringList(rawValue: string | null | undefined) {
  if (!rawValue) {
    return [] as string[];
  }
  try {
...
export default function App() {
  ...
  const [assessmentFeed, setAssessmentFeed] = useState<AssessmentRunLatestItem[]>([]);
  const [selectedAssessmentId, setSelectedAssessmentId] = useState<number | null>(null);
  const [selectedAssessmentDetail, setSelectedAssessmentDetail] = useState<AssessmentRunDetail | null>(null);
  const [pendingProposals, setPendingProposals] = useState<ProposalListItem[]>([]);
  const [selectedProposalId, setSelectedProposalId] = useState<number | null>(null);
  const [selectedProposalDetail, setSelectedProposalDetail] = useState<ProposalDetail | null>(null);
  const [loadingAssessmentFeed, setLoadingAssessmentFeed] = useState(false);
  const [loadingAssessmentDetail, setLoadingAssessmentDetail] = useState(false);
  const [loadingProposalQueue, setLoadingProposalQueue] = useState(false);
  const [loadingProposalDetail, setLoadingProposalDetail] = useState(false);
  ...
  const [operatorIdentity, setOperatorIdentity] = useState("local-operator");
  ...
  useEffect(() => {
    ...
    void loadAssessmentFeedForSeason(selectedSeason.season_id, null);
    void loadPendingProposalsForSeason(selectedSeason.season_id, null);
  ...
  async function loadAssessmentFeedForSeason(seasonId: number, preferredAssessmentId: number | null) {
    try {
      setLoadingAssessmentFeed(true);
      const query = new URLSearchParams({ season_id: String(seasonId) });
      const data = await fetchJson<LatestAssessmentsResponse>(`/api/assessments/latest?${query.toString()}`);
      setAssessmentFeed(data.items);
      const nextAssessmentId =
        preferredAssessmentId ??
        data.items[0]?.assessment_run_id ??
        null;
      ...
      setLoadingAssessmentFeed(false);
    }
  }

  async function loadAssessmentDetail(assessmentRunId: number) {
    try {
      setLoadingAssessmentDetail(true);
      const detail = await fetchJson<AssessmentRunDetail>(`/api/assessments/runs/${assessmentRunId}`);
  ...
  async function loadPendingProposalsForSeason(seasonId: number, preferredProposalId: number | null) {
    try {
      setLoadingProposalQueue(true);
      const query = new URLSearchParams({ season_id: String(seasonId), status: "pending" });
      const data = await fetchJson<ProposalListResponse>(`/api/proposals?${query.toString()}`);
      setPendingProposals(data.items);
      const nextProposalId = preferredProposalId ?? data.items[0]?.proposal_id ?? null;
      ...
      setLoadingProposalQueue(false);
    }
  }

  async function loadProposalDetail(proposalId: number) {
    try {
      setLoadingProposalDetail(true);
      const detail = await fetchJson<ProposalDetail>(`/api/proposals/${proposalId}`);
      setSelectedProposalDetail(detail);
    } catch (requestError) {
      setSelectedProposalDetail(null);
      setError(requestError instanceof Error ? requestError.message : "Error desconocido");
    } finally {
      setLoadingProposalDetail(false);
    }
  }

  async function submitProposalDecision(decisionStatus: "accepted" | "rejected") {
    if (!selectedSeason || !selectedProposalDetail) {
      return;
    }
    ...
    try {
      ...
      setError(null);
      setInfoMessage(null);
      const response = await fetch(`/api/proposals/${selectedProposalDetail.proposal_id}/decision`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          decided_by: operatorIdentity,
          decision_note: proposalDecisionNote || undefined,
        }),
      });
      if (!response.ok) {
        throw new Error(await getApiErrorMessage(response, `Error ${response.status} guardando la decision de la propuesta`));
      }

      setSubmissionMessage(
        `Propuesta ${selectedProposalDetail.proposal_id} ${decisionStatus === "accepted" ? "aceptada" : "rechazada"} por ${operatorIdentity}.`,
      );
      setProposalDecisionNote("");

      await Promise.all([
        loadAssessmentFeedForSeason(selectedSeason.season_id, selectedAssessmentId),
        loadPendingProposalsForSeason(selectedSeason.season_id, selectedProposalDetail.proposal_id),
        loadProposalDetail(selectedProposalDetail.proposal_id),
      ]);

      if (selectedAssessmentId === selectedProposalDetail.source_assessment.assessment_run_id) {
  ...
  {infoMessage ? <section className="info-banner">{infoMessage}</section> : null}
      ...
      <section className="panel assessment-review-panel">
        <div className="section-heading">
          <div>
            <h2>Revision AI</h2>
            <p className="section-subtitle">Ultimas evaluaciones por cadencia, detalle de evidencia y cola de propuestas pendientes sin inferencia en frontend.</p>
          </div>
          <label className="assessment-operator-field">
            <span>Operador</span>
        ...
        <div className="assessment-review-layout">
          <section className="assessment-review-column">
            <div className="assessment-column-head">
              <h3>Ultimas evaluaciones</h3>
              <span className="assessment-column-caption">Resumen por cadencia</span>
            </div>

            {loadingAssessmentFeed ? (
              <div className="empty-state-card">
                <strong>Cargando evaluaciones</strong>
                <p>Recuperando las ultimas ejecuciones persistidas para la temporada activa.</p>
              </div>
            ) : assessmentFeed.length > 0 ? (
              <div className="assessment-list">
                {assessmentFeed.map((item) => (
                  <button
                    key={item.assessment_run_id}
                    type="button"
                    className={`assessment-list-item${selectedAssessmentId === item.assessment_run_id ? " selected" : ""}`}
                    onClick={() => setSelectedAssessmentId(item.assessment_run_id)}
                  >
                    <div className="assessment-list-head">
                      <div>
                        <strong>{item.agent_profile_name}</strong>
                        <p>{toCadenceLabel(item.cadence)} · {item.window_start_date} a {item.window_end_date}</p>
                      </div>
                      <span className={toReviewBadgeClass(item.run_status)}>{toReviewStatusLabel(item.run_status)}</span>
                    </div>
                    <div className="assessment-list-meta">
                      <span>{toConfidenceLabel(item.confidence_label)}</span>
                      <span>{item.pending_proposal_count} pendientes</span>
                      <span>{item.proposal_count} propuestas</span>
                    </div>
                    <p className="assessment-list-summary">{item.summary_text ?? "Sin resumen narrativo persistido."}</p>
                  </button>
                ))}
              </div>
            ) : (
              <div className="empty-state-card">
                ...
                <p>Todavia no hay ejecuciones AI persistidas para la temporada activa.</p>
              </div>
            )}
          </section>
          ...
          <section className="assessment-review-column">
            <div className="assessment-column-head">
              <h3>Detalle de evaluacion</h3>
              <span className="assessment-column-caption">Hallazgos, evidencia y dialogo</span>
            </div>

            {loadingAssessmentDetail ? (
              <div className="empty-state-card">
                <strong>Cargando detalle</strong>
                <p>Recuperando los hallazgos y el contexto persistido de la evaluacion seleccionada.</p>
              </div>
            ) : selectedAssessmentDetail ? (
              <div className="assessment-detail-card">
                <div className="assessment-detail-head">
                  <div>
                    <strong>{selectedAssessmentDetail.agent_profile.display_name}</strong>
                    <p>{toCadenceLabel(selectedAssessmentDetail.agent_profile.cadence)} · {selectedAssessmentDetail.window.subject_scope_key}</p>
                  </div>
                  <div className="assessment-detail-badges">
                    <span className={toReviewBadgeClass(selectedAssessmentDetail.run_status)}>{toReviewStatusLabel(selectedAssessmentDetail.run_status)}</span>
                    <span className="badge badge-pending">{toConfidenceLabel(selectedAssessmentDetail.confidence_label)}</span>
                  </div>
                </div>

                <p className="assessment-detail-summary">{selectedAssessmentDetail.summary_text ?? "Sin resumen narrativo persistido."}</p>

                <div className="summary-strip assessment-summary-strip">
                  <article>
                    <strong>{selectedAssessmentDetail.findings.length}</strong>
                    <span>Hallazgos</span>
                  </article>
                  <article>
                    <strong>{selectedAssessmentDetail.principal_evidence.length}</strong>
                    <span>Evidencias</span>
                  </article>
                  <article>
                    <strong>{selectedAssessmentDetail.proposals.length}</strong>
                    <span>Propuestas</span>
                  </article>
                  <article>
                    <strong>{selectedAssessmentDetail.dialog_context.length}</strong>
                    <span>Entradas de dialogo</span>
                  </article>
                </div>

                <div className="assessment-evidence-group">
                  <h4>Evidencia principal</h4>
                  {selectedAssessmentDetail.principal_evidence.length > 0 ? (
                    <ul className="review-bullet-list">
                      {selectedAssessmentDetail.principal_evidence.map((evidence) => (
                        <li key={evidence}>{evidence}</li>
                      ))}
                    </ul>
                  ) : (
                    <p className="assessment-muted-copy">Sin evidencia principal persistida.</p>
                  )}
                </div>

                <div className="assessment-type-list">
                  {selectedAssessmentDetail.assessment_type_results.map((result) => {
                    const evidenceRows = parseJsonStringList(result.evidence_summary_json);
                    return (
                      <article key={`${result.assessment_type_key}-${result.result_label}`} className="assessment-type-card">
                        <div className="assessment-list-head">
                          <div>
                            <strong>{toHumanLabel(result.assessment_type_key)}</strong>
                            <p>{toHumanLabel(result.result_label)}</p>
                          </div>
                          <span className="badge badge-pending">{toConfidenceLabel(result.confidence_label)}</span>
                        </div>
                        {result.narrative_text ? <p className="assessment-detail-summary">{result.narrative_text}</p> : null}
                        {evidenceRows.length > 0 ? (
                          <ul className="review-bullet-list compact">
                ...
                <div className="assessment-finding-list">
                  {selectedAssessmentDetail.findings.map((finding) => {
                    const evidenceRefs = parseJsonStringList(finding.evidence_refs_json);
                    return (
                      <article key={`${finding.finding_kind}-${finding.sort_order}-${finding.title}`} className="assessment-finding-card">
                        <div className="assessment-list-head">
                          <div>
                            <strong>{finding.title}</strong>
                            <p>{toHumanLabel(finding.finding_kind)}</p>
                          </div>
                          <span className={toReviewBadgeClass(finding.severity ?? "pending")}>{toHumanLabel(finding.severity)}</span>
                        </div>
                        {finding.detail_text ? <p className="assessment-detail-summary">{finding.detail_text}</p> : null}
                ...
                <div className="assessment-evidence-group">
                  <h4>Dialogo acotado</h4>
                  {selectedAssessmentDetail.dialog_context.length > 0 ? (
                    <div className="dialog-entry-list">
                      {selectedAssessmentDetail.dialog_context.map((entry) => (
                        <article key={`${entry.dialog_context_id ?? entry.entry_text}-${entry.created_at ?? ""}`} className="dialog-entry-card">
                          <div className="assessment-list-head">
                            <div>
                              <strong>{toDialogScopeLabel(entry.entry_scope)}</strong>
                              <p>{toHumanLabel(entry.entry_kind)} · {entry.created_by}</p>
                            </div>
                            <span className="badge badge-pending">{toClarificationKindLabel(entry.clarification_kind)}</span>
                          </div>
                          <p className="assessment-detail-summary">{entry.entry_text}</p>
                          {parseJsonStringList(entry.linked_evidence_json).length > 0 ? (
                            <ul className="review-bullet-list compact">
                              {parseJsonStringList(entry.linked_evidence_json).map((evidence) => (
                                <li key={evidence}>{evidence}</li>
                              ))}
                            </ul>
                          ) : null}
                    ...
                    <p className="assessment-muted-copy">Todavia no hay aclaraciones persistidas para esta evaluacion.</p>
                  )}
                </div>

                <div className="assessment-evidence-group">
                  <h4>Propuestas vinculadas</h4>
                  {selectedAssessmentDetail.proposals.length > 0 ? (
                    <div className="proposal-reference-list">
                      {selectedAssessmentDetail.proposals.map((proposal) => (
                        <button
                          key={proposal.proposal_id}
                          type="button"
                          className={`proposal-reference-item${selectedProposalId === proposal.proposal_id ? " selected" : ""}`}
                          onClick={() => setSelectedProposalId(proposal.proposal_id)}
                        >
                          <strong>{proposal.proposal_title}</strong>
                          <span>{toCadenceLabel(proposal.source_cadence)} → {toHumanLabel(proposal.target_planning_level)}</span>
                          <span className={toReviewBadgeClass(proposal.proposal_status)}>{toHumanLabel(proposal.proposal_status)}</span>
                        </button>
                      ))}
                    </div>
                  ) : (
                    <p className="assessment-muted-copy">Esta evaluacion no genero propuestas.</p>
                  )}
                </div>
              </div>
            ) : (
              <div className="empty-state-card">
                <strong>Sin evaluacion seleccionada</strong>
                <p>Elige una de las ultimas evaluaciones para revisar hallazgos, evidencia y dialogo persistido.</p>
              </div>
            )}
          </section>
          ...
          <section className="assessment-review-column">
            <div className="assessment-column-head">
              ...
              <span className="assessment-column-caption">Decision con operador explicito</span>
            </div>

            {loadingProposalQueue ? (
              <div className="empty-state-card">
                <strong>Cargando propuestas</strong>
                <p>Recuperando la cola pendiente de aprobacion de la temporada activa.</p>
              </div>
              ...
              <div className="proposal-queue-list">
                {pendingProposals.map((proposal) => (
                  <button
                    key={proposal.proposal_id}
                    type="button"
                    className={`proposal-queue-item${selectedProposalId === proposal.proposal_id ? " selected" : ""}`}
                    onClick={() => setSelectedProposalId(proposal.proposal_id)}
                  >
                    <div className="assessment-list-head">
                      <div>
                        <strong>{proposal.proposal_title}</strong>
                        <p>{toCadenceLabel(proposal.source_cadence)} → {toHumanLabel(proposal.target_planning_level)}</p>
                      </div>
                      <span className={toReviewBadgeClass(proposal.proposal_status)}>{toHumanLabel(proposal.proposal_status)}</span>
                    </div>
                    <div className="assessment-list-meta">
                      <span>{proposal.agent_profile_key}</span>
                      <span>{toDateTimeLabel(proposal.created_at)}</span>
                    </div>
                    <p className="assessment-list-summary">{proposal.proposal_summary ?? "Sin resumen de propuesta."}</p>
                    {proposal.conflict_group_key ? <small>Conflicto: {proposal.conflict_group_key}</small> : null}
                  </button>
                ))}
              </div>
            ) : (
              <div className="empty-state-card">
                ...
                <p>No hay propuestas pendientes para la temporada activa.</p>
              </div>
            )}

            {loadingProposalDetail ? (
              <div className="empty-state-card">
                <strong>Cargando propuesta</strong>
                <p>Recuperando el detalle completo de la propuesta seleccionada.</p>
              </div>
            ) : selectedProposalDetail ? (
              <div className="proposal-detail-card">
                <div className="assessment-detail-head">
                  <div>
                    <strong>{selectedProposalDetail.proposal_title}</strong>
                    <p>{toCadenceLabel(selectedProposalDetail.source_cadence)} → {toHumanLabel(selectedProposalDetail.target_planning_level)}</p>
                  </div>
                  <span className={toReviewBadgeClass(selectedProposalDetail.proposal_status)}>{toHumanLabel(selectedProposalDetail.proposal_status)}</span>
                </div>

                <p className="assessment-detail-summary">{selectedProposalDetail.proposal_summary ?? "Sin resumen de propuesta persistido."}</p>
                {selectedProposalDetail.reasoning_summary ? (
                  <p className="assessment-muted-copy">Razonamiento: {selectedProposalDetail.reasoning_summary}</p>
                ) : null}

                <div className="proposal-source-card">
                  <strong>Origen</strong>
                  <p>{selectedProposalDetail.source_assessment.agent_profile_key} · {selectedProposalDetail.source_assessment.window_start_date} a {selectedProposalDetail.source_assessment.window_end_date}</p>
                  <button
                    type="button"
                    className="ghost-button"
                    onClick={() => setSelectedAssessmentId(selectedProposalDetail.source_assessment.assessment_run_id)}
                  >
                    Ver evaluacion #{selectedProposalDetail.source_assessment.assessment_run_id}
                  </button>
                </div>

                <div className="assessment-evidence-group">
                  <h4>Cambio propuesto</h4>
                  <pre className="proposal-json-block">{JSON.stringify(selectedProposalDetail.proposed_change, null, 2)}</pre>
                </div>

                <div className="assessment-evidence-group">
                  <h4>Dialogo vinculado</h4>
                  {selectedProposalDetail.dialog_context.length > 0 ? (
                    <div className="dialog-entry-list compact-list">
                      {selectedProposalDetail.dialog_context.map((entry) => (
                        <article key={`${entry.dialog_context_id ?? entry.entry_text}-${entry.created_at ?? ""}`} className="dialog-entry-card compact-card">
                          <strong>{toDialogScopeLabel(entry.entry_scope)}</strong>
                          <p className="assessment-detail-summary">{entry.entry_text}</p>
                        </article>
                      ))}
                    ...
                    <p className="assessment-muted-copy">Esta propuesta no tiene dialogo vinculado persistido.</p>
                  )}
                </div>

                <div className="assessment-evidence-group">
                  <h4>Historial de decisiones</h4>
                  {selectedProposalDetail.decision_history.length > 0 ? (
                    <div className="decision-history-list">
                      {selectedProposalDetail.decision_history.map((decision) => (
                        <article key={decision.proposal_decision_id} className="decision-history-card">
                          <div className="assessment-list-head">
                            <strong>{toHumanLabel(decision.decision_status)}</strong>
                            <span className={toReviewBadgeClass(decision.decision_status)}>{toDateTimeLabel(decision.decided_at)}</span>
                          </div>
                          <p className="assessment-muted-copy">{decision.decided_by}{decision.applied_change_ref ? ` · ${decision.applied_change_ref}` : ""}</p>
                          {decision.decision_note ? <p className="assessment-detail-summary">{decision.decision_note}</p> : null}
                        </article>
                      ))}
                    </div>
                  ) : (
                    <p className="assessment-muted-copy">Sin decisiones persistidas todavia. El plan canonico sigue intacto hasta aceptar.</p>
                  )}
                </div>

                <label className="manual-form">
                  <span>Nota de decision</span>
                  <textarea
                    rows={3}
                    value={proposalDecisionNote}
                    onChange={(event) => setProposalDecisionNote(event.target.value)}
                ...
                <div className="review-actions">
                  <button
                    ...
                    type="button"
                    onClick={() => void submitProposalDecision("accepted")}
                    disabled={savingProposalDecision || selectedProposalDetail.proposal_status !== "pending"}
                  >
                    {savingProposalDecision && selectedProposalDetail.proposal_status === "pending" ? "Guardando..." : "Aprobar propuesta"}
                  </button>
                  <button
                    className="ghost-button"
                    type="button"
                    onClick={() => void submitProposalDecision("rejected")}
                    disabled={savingProposalDecision || selectedProposalDetail.proposal_status !== "pending"}
                  >
                    Rechazar propuesta
                  </button>
