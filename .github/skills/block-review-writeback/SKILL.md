...
3. If the request arrives via handoff from `Block Performance Coach`, treat the handed-off response body as the default `detailed_assessment_markdown` with no manual rewriting.
4. Keep the persisted markdown fully in English. If the source assessment contains Spanish block text or weekly evidence snippets, translate them before persistence unless a brief direct quote is necessary.
...
7. If the request is a persistence handoff from `Block Performance Coach` and there is no read-only or dry-run override, run write mode directly.
8. Otherwise run:
   [upsert_block_review.py](./scripts/upsert_block_review.py)
...
- Translate Spanish planning text, block notes, and source evidence into natural English before writing.
...
- Prefer explicit `block_id` and `season_id` when known.
- If derived fields are omitted, infer them only when the target block resolves unambiguously.
